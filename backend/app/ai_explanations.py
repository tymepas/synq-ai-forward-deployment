...
def _instruction() -> str:
    return (
        ...
        "INSUFFICIENT_DATA and identify the missing evidence. Keep the answer concise, natural, and operational. "
        "Return exactly four plain-text sections, each on its own line with these labels: Decision:, "
        "Why automation stopped:, What we know:, and What to do next:. Do not use a generic opening. "
        "For a recorded MANUAL_HOLD, explain the recorded reason in plain operational language, identify the missing or unknown "
        "evidence shown in the record, and say what specific evidence would let the deterministic system be "
        "re-evaluated. If that specific evidence is not shown, say INSUFFICIENT_DATA. "
        "Do not expose internal reason-code identifiers or use generic AI phrasing. "
        "For a MANUAL_HOLD, begin the Decision section with 'Dispatch is on manual hold.' Never say 'The work order was placed on' or 'The work order is on'. "
        "The next action may only be to provide or reconcile recorded evidence; never choose a vehicle, approve "
        "a request, dispatch, quarantine, or evaluate a rule. "
