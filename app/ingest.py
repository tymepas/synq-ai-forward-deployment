...
def ingest_tickets(settings: Settings, store: ContextStore, redactor: PIIRedactor | None = None) -> dict[str, int]:
    """Ingest the primary queue without persisting unredacted ticket content."""
    source_path = settings.corpus_dir / "tickets.json"
    ...
    source_id = store.upsert_source("tickets.json", file_sha256(source_path), len(records))
    counts = {"records": len(records), "valid": 0, "quarantined": 0, "duplicates": 0}

    for record_index, raw in enumerate(records, start=1):
        # A same-content source re-ingested on a rerun is already fully represented,
        # including its audit trail. Skipping it keeps every exported artifact stable.
        if store.ticket_record_exists(source_id, record_index):
            continue
