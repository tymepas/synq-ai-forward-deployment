...
export default function QuarantinePage() {
  ...
  return <>
    ...
    <p className="page-intro">These records were intentionally blocked from automation. They remain available for safe review and remediation.</p><div className="toolbar single"><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search ticket or reason code" aria-label="Search quarantine" /></div>
    {loading ? <LoadingPanel /> : error ? <ErrorPanel message={error} onRetry={load} /> : filtered.length === 0 ? <EmptyPanel title="No quarantine records" detail="The currently loaded data has no invalid or ambiguous inputs." /> : <section className="quarantine-list">{filtered.map((item) => <article className="card quarantine-card" key={item.quarantine_id}><div className="card-heading"><div><p className="eyebrow">QUARANTINE REFERENCE</p><h2>{item.ticket_id ?? "File-level record"}</h2></div><span className="pill warning">QUARANTINED</span></div><section className="approval-reason"><strong>Review reason</strong><div className="reason-list">{item.reasons.map((reason) => <span key={reason}>{reason.replaceAll("_", " ")}</span>)}</div><p>Review the recorded reason and correct the source record before re-running deterministic evaluation.</p></section><details><summary>View safe review details</summary><pre>{JSON.stringify(item.summary, null, 2)}</pre></details></article>)}</section>}
  </>;
}
